    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <img src="/doctor.jpg" alt="Doctor Logo" style={{ width: 72, height: 72, borderRadius: 16, margin: '0 auto 16px', display: 'block', objectFit: 'cover' }} />
          <h1 style={{ fontSize: 24, fontWeight: 700 }}>Welcome Back</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 18, fontWeight: 600, marginTop: 4 }}>Doctor</p>
        </div>
        {error && (
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="email" className="input-field" style={{ paddingLeft: 40 }} placeholder="doctor@hospital.com" value={email} onChange={(e) => setEmail(e.target.value)} required />
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 24 }}>
          ...
          <Link to="/patient-login" className="login-link-btn">Patient Portal</Link>
        </div>
      </motion.div>
    </div>
  )
